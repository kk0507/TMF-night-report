...
## 定時報價（scheduled_quote.py）

除了每天07:00的完整分析快報，另外三個時間點會送一則簡單報價（不含百分位/均線分析，純粹「現在多少錢」）：

- **13:50**（日盤13:45收盤後）
- **20:50**（夜盤中）
- **05:05**（夜盤05:00收盤後，隔天）

三個排程共用同一支`scheduled_quote.py`跟同一個workflow(`scheduled_quote.yml`)，用GitHub Actions的`github.event.schedule`判斷這次該送哪一種文字標籤。
